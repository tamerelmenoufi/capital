...
$data->read($caminho . $nomeAleatorio);

	//Este for representa a quantidade de linhas do excel
	for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {
		
			//for ($j = 1; $j <= 3/*$data->sheets[0]['numCols']*/; $j++) {
				$nome = utf8_encode((!empty($data->sheets[0]['cells'][$i][1])) ? $data->sheets[0]['cells'][$i][1] : false);
				$email = utf8_encode((!empty($data->sheets[0]['cells'][$i][2])) ? $data->sheets[0]['cells'][$i][2] : false);
				$telefone = utf8_encode((!empty($data->sheets[0]['cells'][$i][3])) ? $data->sheets[0]['cells'][$i][3] : false);
				
				if($telefone or $email){
					$p = array(".","/","-");
					$telefone = str_replace($p,false,TratarNumero($telefone));
					$ddd = substr($telefone,0,2);
					$num = substr($telefone,2,10);
					
					list($CheckMail) = mysql_fetch_row(mysql_query("select count(*) from ".$_POST[tabela]." where email='".$email."' and categoria='".$_POST[categoria]."'"));
					list($CheckFone) = mysql_fetch_row(mysql_query("select count(*) from ".$_POST[tabela]." where cod_pais='55' and cod_estado='".$ddd."' and telefone='".$num."' and categoria='".$_POST[categoria]."'"));
					
					if(!$CheckMail or !$CheckFone){
					mysql_query("insert into ".$_POST[tabela]." set categoria='".$_POST[categoria]."', nome='".$nome."', email='".$email."', cod_pais='55', cod_estado='".$ddd."', telefone='".$num."', situacao='1'");
					}
				}
